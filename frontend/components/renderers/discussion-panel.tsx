          ...
          {showTranscriptControls ? (
            <div className={styles.transcriptControlsBlock}>
              <div className={styles.commandGrid}>
                ...
                <button
                  className={`buttonGhost ${styles.restartBtn}`.trim()}
                  type="button"
                  onClick={() => void onWipeDebate()}
                  disabled={isWipingSession || discussionActive}
                  aria-label="Restart debate"
                >
                  <span className={styles.restartBtnIcon} aria-hidden="true">
                    <RestartIcon size={15} />
                  </span>
                  <span>{isWipingSession ? "Restarting..." : "Restart"}</span>
                </button>
